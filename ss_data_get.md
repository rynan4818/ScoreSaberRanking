...
上記APIでの取得と利用は(他サイトやローカルHTMLファイルからの)ブラウザのJavaScriptからは[CORS](https://www.google.co.jp/search?q=CORS)によってアクセスできませんので注意して下さい。ブラウザ以外のプログラムからのアクセスは問題ありません。 ※BeatSavarのAPIなどサーバー側でアクセス許可されているものはオーバーレイなどで使用可能です。